...
const buildTestSupportTree = memoize(function buildTestSupportTree() {
  // Addons like ember-window-mock rely on test-support modules
  // overwriting regular modules, so we can't use nonDuplicatedAddonInvoke
  // because it may lead to 'addon' trees not having the corresponding
  // 'addon-test-support' trees
  let childAddonsAddonTestSupportTrees = this.eachAddonInvoke('treeFor', ['addon-test-support']);
  let childAddonsAddonTestSupportTreesMerged = mergeTrees(childAddonsAddonTestSupportTrees, {
    overwrite: true,
  });

  return childAddonsAddonTestSupportTreesMerged;
});
...
const buildCompleteJSTree = memoize(function buildCompleteJSTree() {
  ...
  if (this.tests) {
    let testSupportTree = buildTestSupportTree.call(this);
...
module.exports = {
  extend: function(options) {
    ...
    options.init = function() {
      ...
      // taken from https://github.com/ember-cli/ember-cli/blob/v3.0.3/lib/broccoli/ember-app.js#L120
      // needed for test-support
      ...
      this.treeForPublic = function() {
        ...
        // taken from https://github.com/ember-cli/ember-cli/blob/v3.0.3/lib/broccoli/ember-app.js#L1811
        if (this.tests) {
          testSupportTree = buildTestSupportTree.call(this);
        }
        ...
        if (!this.options || !this.options.babel) {
          ...
          if (testSupportTree) {
            testSupportTree = processBabel(testSupportTree);
          }
        }
        ...
        if (testSupportTree) {
          let concatTestSupportTree = concat(testSupportTree, {
            allowNone: true,
            inputFiles: ['**/*.js'],
            outputFile: 'engines-dist/' + this.name + '/assets/test-support.js',
          });

          finalMergeTrees.push(concatTestSupportTree);
        }
